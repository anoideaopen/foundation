...
func TestEmitTransfer(t *testing.T) {
	...
	testCollection := []struct {
		...
		funcPrepareMockStub func(
			...
			owner, feeSetter, feeAddressSetter, feeAggregator, user1, user2 *mocks.UserFoundation) []string
		funcInvokeChaincode func(
			cc *core.Chaincode,
			mockStub *mockstub.MockStub,
			owner, feeSetter, feeAddressSetter, feeAggregator, user1, user2 *mocks.UserFoundation,
			parameters ...string) peer.Response
	...
	}{
		{
			name:         "Emit tokens",
			functionName: "emit",
			funcPrepareMockStub: func(t *testing.T, mockStub *mockstub.MockStub, owner, feeSetter, feeAddressSetter, feeAggregator, user1, user2 *mocks.UserFoundation) []string {
				return []string{user1.AddressBase58Check, "1000"}
			},
			funcInvokeChaincode: func(cc *core.Chaincode, mockStub *mockstub.MockStub, owner, feeSetter, feeAddressSetter, feeAggregator, user1, user2 *mocks.UserFoundation, parameters ...string) peer.Response {
				_, resp := mockStub.TxInvokeChaincodeSigned(cc, "emit", owner, "", "", "", parameters...)
				return resp
			},
			funcCheckResponse: func(t *testing.T, mockStub *mockstub.MockStub, resp peer.Response) {
				userBalanceKey, err := mockStub.CreateCompositeKey(balance.BalanceTypeToken.String(), []string{user1.AddressBase58Check})
				require.NoError(t, err)

				checked := false
				for i := 0; i < mockStub.PutStateCallCount(); i++ {
					putStateKey, value := mockStub.PutStateArgsForCall(i)
					if putStateKey == userBalanceKey {
		...
		},
		{
			...
			funcInvokeChaincode: func(cc *core.Chaincode, mockStub *mockstub.MockStub, owner, feeSetter, feeAddressSetter, feeAggregator, user1, user2 *mocks.UserFoundation, parameters ...string) peer.Response {
				_, resp := mockStub.TxInvokeChaincodeSigned(cc, "setFeeAddress", feeAddressSetter, "", "", "", parameters...)
				return resp
			},
			funcCheckResponse: func(t *testing.T, mockStub *mockstub.MockStub, resp peer.Response) {
				...
				for i := 0; i < mockStub.PutStateCallCount(); i++ {
					...
					if putStateKey == keyMetadata {
						...
						require.Equal(t, feeAggregator.AddressBase58Check, string(tokenMetadata.FeeAddress))
	...
	for _, test := range testCollection {
		t.Run(test.name, func(t *testing.T) {
			parameters := test.funcPrepareMockStub(t, mockStub, owner, feeSetter, feeAddressSetter, feeAggregator, user1, user2)
			resp := test.funcInvokeChaincode(cc, mockStub, owner, feeSetter, feeAddressSetter, feeAggregator, user1, user2, parameters...)
			require.Equal(t, int32(http.StatusOK), resp.GetStatus())
			require.Empty(t, resp.GetMessage())
			test.funcCheckResponse(t, mockStub, resp)
		})
	}

	/*
		_, resp := mockStub.TxInvokeChaincodeSigned(cc, "emit", owner, "", "", "", []string{user1.AddressBase58Check, "1000"}...)
		require.Equal(t, int32(http.StatusOK), resp.GetStatus())
		require.Empty(t, resp.GetMessage())
		...
		_, resp = mockStub.TxInvokeChaincodeSigned(cc, "setFeeAddress", feeAddressSetter, "", "", "", feeAggregator.AddressBase58Check)
		require.Equal(t, int32(http.StatusOK), resp.GetStatus())
		require.Empty(t, resp.GetMessage())
		...
		for i := 0; i < mockStub.PutStateCallCount(); i++ {
			...
			if putStateKey == keyMetadata {
				...
				err := json.Unmarshal(value, tokenMetadata)
				require.NoError(t, err)

				require.Equal(t, feeAggregator.AddressBase58Check, tokenMetadata.FeeAddress)
		...
		_, resp := mockStub.TxInvokeChaincodeSigned(cc, "setFee", feeSetter, "", "", "", []string{"FIAT", "500000", "100", "100000"}...)
		require.Equal(t, int32(http.StatusOK), resp.GetStatus())
		require.Empty(t, resp.GetMessage())
